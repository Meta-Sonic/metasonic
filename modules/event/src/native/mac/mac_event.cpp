...
public:
  ...
  inline void run() { CFRunLoopRun(); }

  inline void stop() { CFRunLoopStop(_run_loop); }
...
void event_manager::run() { _pimpl->run(); }

void event_manager::stop() { _pimpl->stop(); }
