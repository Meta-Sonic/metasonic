...
AudioDeviceID get_default_output_device_id(std::error_code& ec) {
  UInt32 dataSize = sizeof(AudioDeviceID);
...
AudioDeviceID get_default_input_device_id(std::error_code& ec) {
  UInt32 dataSize = sizeof(AudioDeviceID);
...
audio_device_info get_audio_device_info_from_id(AudioDeviceID device_id, std::error_code& ec) {
  audio_device_manager::device_info dinfo;
  dinfo.id = (audio_device_id)device_id;
  ...
  if (dinfo.output_channels > 0) {
    if (get_default_output_device_id(ec) == device_id) {
  ...
  if (dinfo.input_channels > 0) {
    if (get_default_input_device_id(ec) == device_id) {
...
audio_device_info get_audio_device_info(
    audio_device_index index, const std::vector<AudioDeviceID>& device_list, std::error_code& ec) {
  ...
  return get_audio_device_info_from_id(device_id, ec);
}
...
// implementation.
struct CoreHandle {
  AudioDeviceID id[2] = { kAudioObjectUnknown, kAudioObjectUnknown }; // device ids
  AudioDeviceIOProcID procId[2] = { nullptr, nullptr };
  UInt32 iStream[2] = { 0, 0 }; /// device stream index (or first if using multiple)
  UInt32 nStreams[2] = { 1, 1 }; /// number of streams to use
  bool xrun[2] = { false, false };
  void* deviceBuffer = nullptr;
  std::condition_variable condition;
  int drainCounter = 0; /// Tracks callback counts when draining
  bool internalDrain = false; /// Indicates if stop is initiated from callback or not.
  bool xrunListenerAdded[2] = { false, false };
  bool disconnectListenerAdded[2] = { false, false };
};
} // namespace.
...
struct core_audio_engine::impl {
  ...
  // function is called.
  static void coreStopStream(callback_info* info) {
  ...
  static bool callbackEvent(core_audio_engine* object, AudioDeviceID deviceId, const AudioBufferList* inBufferList,
    ...
    callback_info* info = (callback_info*)&object->_stream.callbackInfo;
    ...
    if (handle->drainCounter == 0 && (object->_stream.mode != DUPLEX || deviceId == outputDevice)) {
      ...
      // Call user callback.
      mts::audio_device_manager::callback_result cbReturnValue = info->callback(object->_stream.userBuffer[0],
...
std::error_code core_audio_engine::init() {
  ...
  AudioObjectPropertyAddress property = make_property_address(kAudioHardwarePropertyRunLoop);
...
audio_device_info core_audio_engine::get_audio_device_info(audio_device_index index, std::error_code& ec) {
  std::vector<AudioDeviceID> device_list = get_device_list(ec);
...
audio_device_info core_audio_engine::get_audio_device_info(audio_device_id device_id, std::error_code& ec) {
  return ::mts::get_audio_device_info_from_id((AudioDeviceID)device_id, ec);
}
...
void core_audio_engine::close_stream() {
  ...
  if (_stream.mode == OUTPUT || _stream.mode == DUPLEX) {
    if (handle) {
      ...
      if (handle->xrunListenerAdded[0]) {
        ...
        if (AudioObjectRemovePropertyListener(handle->id[0], &property, &impl::xrunListener, (void*)handle) != noErr) {
      ...
      if (handle->disconnectListenerAdded[0]) {
        ...
        if (AudioObjectRemovePropertyListener(
                handle->id[0], &property, &impl::disconnectListener, (void*)&_stream.callbackInfo)
            != noErr) {
  ...
  if (_stream.mode == INPUT || (_stream.mode == DUPLEX && _stream.device[0] != _stream.device[1])) {
    if (handle) {
      ...
      if (handle->xrunListenerAdded[1]) {
        property.mSelector = kAudioDeviceProcessorOverload;
        if (AudioObjectRemovePropertyListener(handle->id[1], &property, &impl::xrunListener, (void*)handle) != noErr) {
          //            errorText_ = "RtApiCore::closeStream(): error removing xrun property listener!";
          //            error( RTAUDIO_WARNING );
        }
      }
      ...
      if (handle->disconnectListenerAdded[0]) {
        property.mSelector = kAudioDevicePropertyDeviceIsAlive;
        if (AudioObjectRemovePropertyListener(
                handle->id[1], &property, &impl::disconnectListener, (void*)&_stream.callbackInfo)
            != noErr) {
          //            errorText_ = "RtApiCore::closeStream(): error removing disconnect property listener!";
          //            error( RTAUDIO_WARNING );
        }
  ...
  callback_info* info = (callback_info*)&_stream.callbackInfo;
...
mts::error_result core_audio_engine::probe_device_open(std::size_t device, stream_mode mode, std::size_t channels,
  ...
  _stream.userBuffer[mode] = malloc(bufferBytes * sizeof(char));

  if (!_stream.userBuffer[mode]) {
  ...
  if (_stream.doConvertBuffer[mode] && handle->nStreams[mode] > 1) {
    ...
    if (makeBuffer) {
      ...
      _stream.deviceBuffer = calloc(bufferBytes, sizeof(char));
