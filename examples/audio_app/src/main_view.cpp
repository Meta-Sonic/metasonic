...
main_view::main_view(audio_device* device)
    : _device(device)
    , _button_1("a")
    , _view_1("v1")
    , _view_2("v2") {
  ...
  const std::vector<mts::audio_device_info>& devices = _device->get_device_list();

  for (std::size_t i = 0; i < devices.size(); i++) {

    if (devices[i].output_channels) {
      _box.add_item(devices[i].name);
    }
    if (devices[i].input_channels) {
      _input.add_item(devices[i].name);
    }
  }
  ...
  add(_box, { 300, 200, 300, 25 });
  add(_input, { 300, 240, 300, 25 });
  ...
  _view_2.add_geometry<mts::shape::filled_rounded_rectangle_with_contour>(
      mts::rect<float>{ 2, 2, 46, 46 }, 0xFF0000FF, 0x000000FF, 2, 5);
  ...
  _button_1.on_click = [this]() {
    ...
    _device->set_output_device(2, 44100, 1024);
    settings_view::show(*this, { 200, 200 });

    //        _settings_window
    //        mts::managed_ptr<popup_window> create(
  ...
  ////    }
  //
  //    mts::optional_call(get_child("v1.a").get(), &mts::view::set_attribute<mts::style::button::background_color>,
  //    mts::color(0xFF0000FF));
}
...
void main_view::property_changed(const std::string& name) { mts::print("property_changed", name, get_property(name)); }
